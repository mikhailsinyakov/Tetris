...
class EventsHandler {
    ...
    addKeyListeners() {
      ...
      this.keyboardListener.addKey('ArrowUp', {once: true}, 
                                  () => this.actions.rotate = 'clockwise');
      this.keyboardListener.addKey('ArrowLeft', {delay: 200}, 
                                  () => this.actions.moveLeft = true);
      this.keyboardListener.addKey('ArrowRight', {delay: 200}, 
                                  () => this.actions.moveRight = true);
      this.keyboardListener.addKey('ArrowDown', {delay: 0}, 
                                  () => this.actions.speedUp = {toBottom: false});
      this.keyboardListener.addKey('Space', {once: true}, 
      ...
      this.touchListener.listen(({direction, touchBottom = false}) => {
		...
		addMouseListener() {
			this.mouseListener.listen(action => {
				...
				else if (action.type === 'mousemove') this.actions.moveTo = {cursor: action.coords};
