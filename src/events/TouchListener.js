...
class TouchListener {
  ...
  getDirection(from, to) {
    ...
    if (
      ...
      Math.abs(deltaX) > cellWidth * 0.9
    ) {
      return deltaX > 0 ? "right" : "left";
    } else if (
      Math.abs(deltaY) > Math.abs(deltaX) &&
      Math.abs(deltaY) > cellWidth * 0.9
    ) {
  ...
  listen(fn) {
    ...
    document.body.addEventListener("touchmove", this.listener, {
      passive: false
    });
    document.body.addEventListener("touchend", this.listener, {
      passive: false
    });
    this.callback = fn;
  }
