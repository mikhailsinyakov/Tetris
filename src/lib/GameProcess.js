...
class GameProcess {
  ...
  finish() {
    ...
    this.records
      .add(Math.round(this.state.info.score), this.state.username)
      .then(records => {
  ...
  updateShadow() {
    store.dispatch(
      updateShapeShadow(this.state.activeShape, this.state.filledCells)
    );
  }
  ...
  updateCursor(cursor) {
    if (
      cursor.x >= 0 &&
      cursor.x < config.fieldSize.width &&
      cursor.y >= 0 &&
      cursor.y < config.fieldSize.height
    ) {
      this.cursor = { x: cursor.x, outside: false };
    } else if (!this.cursor.outside)
      this.cursor = { x: cursor.x, outside: true };
  }
  ...
  speedUp({ toBottom }) {
    ...
    if (!toBottom)
      this.futureActions.push(() => (this.updateMode = "standart"));
  }

  rotate({ clockwise }) {
    store.dispatch(rotateShape(store.getState().filledCells, clockwise));
    this.updateShadow();
  }
